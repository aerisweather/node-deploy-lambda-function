...
async function deployLambdaFunction(params: IDeployParams) {
    ...
    if (params.envFile !== undefined) {
        console.log(` - Updating environment params from supplied S3 file: ${params.envFile}`);
...
async function moveDir(src: string, dst: string) {
    await new Promise((onRes, onErr) => {
        fs.move(src, dst, (err: Error) => err ? onErr(err) : onRes())
    });
}
...
export interface IDeployParams {
    envFile?: string; // location of .env file on s3, otherwise won't touch env vars (like if already set via Terraform)
    srcDirs: string[];
